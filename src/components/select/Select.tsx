...
const Select = (props: SelectProps<any, any[]>, ref: React.Ref<BaseSelectRef>) => {
  const {
    id,
    mode,
    combobox,
...
const WrappedSelect = React.forwardRef(Select);

if (process.env.NODE_ENV !== 'production') {
  WrappedSelect.displayName = 'Select';
}

const TypedSelect = WrappedSelect as unknown as <
  ValueType = any,
