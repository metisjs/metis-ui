...
class Field extends React.Component<InternalFieldProps, FieldState> implements FieldEntity {
  ...
  private cancelRegisterFunc:
    | ((isListField?: boolean, preserve?: boolean, namePath?: InternalNamePath) => void)
    | null = null;
  ...
  private validatePromise: Promise<string[]> | null | undefined;
  ...
  constructor(props: InternalFieldProps) {
    ...
    if (props.fieldContext) {
      const { _getInternalHooks }: InternalFormInstance = props.fieldContext;
      const { initEntityValue } = _getInternalHooks();
  ...
  public componentDidMount() {
    ...
    // Register on init
    if (fieldContext) {
      const { _getInternalHooks }: InternalFormInstance = fieldContext;
      const { registerField } = _getInternalHooks();
  ...
  public isFieldDirty = () => {
    ...
    const { getInitialValue } = fieldContext._getInternalHooks();
  ...
  public getControlled = (childProps: ChildProps = {}) => {
    ...
    const namePath = this.getNamePath();
    const { _getInternalHooks, getFieldsValue }: InternalFormInstance = fieldContext;
    const { dispatch } = _getInternalHooks();
  ...
  public render() {
    ...
    if (isFunction) {
      ...
      const warning = devUseWarning('Form');
      warning(!child, 'usage', '`children` of Field is not validate ReactElement.');
      returnChildNode = child;
    }
...
function WrapperField<Values = any>({
  name,
  ...restProps
}: PartialWith<FieldProps<Values>, 'trigger' | 'valuePropName'>) {
  ...
  if (
    ...
    const warning = devUseWarning('Form');
    warning(false, 'usage', '`preserve` should not apply on Form.List fields.');
  }
