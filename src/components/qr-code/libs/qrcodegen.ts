...
export class QRCode {
  ...
  public static encodeSegments(
    ...
    for (version = minVersion; ; version++) {
      const dataCapacityBits: number = QRCode.getNumDataCodewords(version, oriEcl) * 8; // Number of data bits available
  ...
  private finderPenaltyAddHistory(oriCurrentRunLength: number, runHistory: number[]): void {
    let currentRunLength = oriCurrentRunLength;
