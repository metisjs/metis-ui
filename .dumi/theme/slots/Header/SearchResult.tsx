import React, { Fragment, useCallback, useEffect, useRef, useState, type FC } from 'react';
import { history, useLocation, type useSiteSearch } from 'dumi';
import { clsx, List, ListRef, Skeleton } from 'metis-ui';
...
const SearchResult: FC<{
  ...
  const loadingPlaceholderData = [
    { type: 'title', value: { title: 'skeleton_0' } },
    { type: 'title', value: { title: 'skeleton_1' } },
  ] as ISearchFlatData;

  return (
    <List
      ref={listRef}
      dataSource={props.loading ? loadingPlaceholderData : data}
      virtual={false}
      rowKey={(item) => (item.type === 'title' ? item.value.title! : item.value.link!)}
      renderItem={(item) => {
        if (props.loading) {
          return (
            <List.Item>
        ...
        if (item.type === 'title') {
          return <div className="p-2 font-semibold">{item.value.title}</div>;
        }

        return (
          <List.Item
            onClick={() => onItemSelect(item.value)}
            className={clsx('hover:bg-fill-quaternary cursor-pointer', {
              'bg-fill-quaternary': activeIndex === item.activeIndex,
            })}
          >
            <List.Item.Meta
      ...
      className={{
        item: {
          root: 'px-4',
          meta: { avatar: '*:text-text-tertiary *:size-10', title: 'font-normal' },
        },
      }}
    />
  );
